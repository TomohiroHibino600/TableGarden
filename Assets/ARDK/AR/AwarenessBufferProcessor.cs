...
namespace Niantic.ARDK.AR.Awareness
{
    ...
    where TBuffer: class, IAwarenessBuffer, IDisposable
  {
    ...
    /// @param unityCamera The rendering Unity camera driven by AR pose updates.
    protected void ProcessFrame(TBuffer buffer, IARCamera arCamera, UnityEngine.Camera unityCamera)
    {
      if (unityCamera == null)
      {
        const string errorLog = "No Camera was provided for the ARContextAwarenessProcessor.";
        ARLog._Error(errorLog);
        return;
      }
      ...
      var isDisplayTransformDirty = isFirstUpdate || (_lastOrientation != currentOrientation);
      if (isDisplayTransformDirty)
      {
        ...
        _displayTransform = AwarenessBuffer.CalculateDisplayTransform(unityCamera, _lastOrientation);
      }
      ...
      if (didUpdateAwarenessBuffer)
      {
        ...
        // screen coordinates to 3D points in camera space.
        ...
        // allow the matrix to be used with normalized screen coordinates
      ...
      // matching the orientation of the buffer. It is used
      // to transform points from camera space to world space.
      CameraToWorldTransform = AwarenessBuffer.CalculateCameraToWorldTransform
      (
        unityCamera,
        currentOrientation
      );
