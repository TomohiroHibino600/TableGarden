...
#region IARFrameRenderer Overrides
    ...
    public void AddFeatureProvider(IRenderFeatureProvider provider)
    {
      ...
      // Listen to when the provider enables
      // or a disables render features
...
#region Abstraction Layer
    ...
    /// @param resolution The resolution of the artifact. Either the camera's
    ///                   rendering resolution or the target texture's dimensions.
    protected ARFrameRenderer(RenderTarget target, Resolution resolution)
    {
      Target = target;
    ...
    protected ARFrameRenderer(RenderTarget target, Resolution resolution, float near, float far)
    {
      Target = target;
      Resolution = resolution;
      NearPlane = near;
    ...
    public void Enable()
    {
      ...
      if (Target.IsTargetingCamera)
      {
        // Todo: onPostRender does not work with URP
