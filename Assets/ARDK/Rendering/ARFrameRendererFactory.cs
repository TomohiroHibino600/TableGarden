...
namespace Niantic.ARDK.Rendering
{
  public static class ARFrameRendererFactory
  {
    ...
    {
      return _Create(target, resolution, null, nearClipping, farClipping);
    }
    ...
    )
    {
      ...
      if (envs != null)
      {
        ...
        {
          var possibleResult = Create(target, resolution, env, nearClipping, farClipping);
      ...
      if (!triedAtLeast1)
        return _Create(target, resolution, ARSessionFactory._defaultBestMatches, nearClipping, farClipping);
    ...
    )
    {
      ...
      switch (env)
      {
        case RuntimeEnvironment.Default:
          return Create(target, resolution, nearClipping, farClipping);
...
#if UNITY_IOS
          result = new _ARKitFrameRenderer(target, resolution, nearClipping, farClipping);
#elif UNITY_ANDROID
          result = new _ARCoreFrameRenderer(target, resolution, nearClipping, farClipping);
#else
          return null;
#endif
          break;
#pragma warning restore CS0162
        ...
        case RuntimeEnvironment.Remote:
          ...
          result = new _RemoteFrameRenderer(target, resolution, nearClipping, farClipping);
          break;

        case RuntimeEnvironment.Mock:
          result = new _MockFrameRenderer(target, resolution, nearClipping, farClipping);
          break;
