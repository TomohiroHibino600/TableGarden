...
namespace Niantic.ARDK.VirtualStudio.Editor
{
  ...
  internal sealed class _MockPlayConfigurationEditor
  {
    ...
    {
      args.Networking.Connected += connectedArgs =>
      {
        if (args.Networking is _MockMultipeerNetworking mockNetworking)
        {
          ...
          _sessionMetadata = _detectedSessionMetadata;
        }
      };
    }
    ...
    public void DrawGUI()
    {
      ...
      {
        _playConfiguration.ConnectAllPlayersNetworkings(GetInputSessionMetadata());
        ...
        using (var col2 = new EditorGUILayout.VerticalScope())
        {
          ...
          else
          {
            if (GUILayout.Button("Join", style))
              networking.Join(GetInputSessionMetadata());
          }
    ...
    private byte[] GetInputSessionMetadata()
    {
      if (_sessionMetadata == null)
        _sessionMetadata = Encoding.UTF8.GetBytes(_inputSessionIdentifier);
