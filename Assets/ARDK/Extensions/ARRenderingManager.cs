...
namespace Niantic.ARDK.Helpers
{
    ...
    UnityLifecycleDriver
  {
    ...
    private void ARSessionFactory_SessionInitialized(AnyARSessionInitializedArgs args)
    {
      ...
      {
        var layerMask = LayerMask.GetMask(_MockFrameBufferProvider.MOCK_LAYER_NAME);
        if (layerMask > 0)
          _camera.cullingMask ^= layerMask;
        else
          ARLog._Error(_MockFrameBufferProvider.MOCK_LAYER_MISSING_MSG);
      }
    }
    ...
    private ARFrameRenderer CreateRenderer(RuntimeEnvironment environment)
    {
      ...
      var resolution = isOffscreen
        ? new Resolution
        {
          width = _targetTexture.width, height = _targetTexture.height
        }
        : new Resolution
        {
          width = _camera.pixelWidth, height = _camera.pixelHeight
        };
